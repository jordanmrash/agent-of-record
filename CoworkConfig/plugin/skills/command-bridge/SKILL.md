...
30 rule(s) of 35 entries routed to this skill, generated from
...
| `git-protected-branch-bypass-not-reversible` | Read the branch's protection rules before pushing to it. An admin bypass that succeeds does not imply an undo that succeeds. |
...
| `plugin-install-is-upload-not-open` | Install a built `.plugin` through Customize → Plugins → Add → Upload plugin. Never tell anyone to open or double-click the file. |
