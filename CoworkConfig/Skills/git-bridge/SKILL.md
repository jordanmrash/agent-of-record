...
description: >-
  ...
  (use playwright-skill), or for any GitHub or remote operation on the WORKING repository,
  which is local only. The published repository is a separate, disposable snapshot with its own remote.
cowork:
  category: automation
  icon: BranchFork
metadata:
...
Redundant when the session can run git through the approved executor with the host's own file tools alongside, as Claude Cowork can. Needed on Copilot Cowork, where git runs only inside approved jobs and this skill holds the job shapes and refusal rules.
...
11 rule(s) of 12 entries routed to this skill, generated from
...
- **Never add a remote to the working repository and never push it.** No `git remote add`, no
  `git push`, no `git clone` into this repository. Firm data must not leave the machine through a
  git remote — this is the single most important rule in this skill. Publication happens only
  through the clean-room snapshot described in `GitHubSetup/PUBLISHING.md`, which is a different
